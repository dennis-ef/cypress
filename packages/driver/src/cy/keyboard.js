...
const countNumIndividualKeyStrokes = (keys) => {
  ...
  const kb = {
    ...
    simulateKey (el, eventType, key, options) {
      ...
      if (otherKeys) {
        ...
        mixinModifiers(event, kb.getActiveModifiers())
      }
    ...
    handleModifier (el, chars, options) {
      ...
      if (state('keyboardModifiers')[modifier]) {
        return
      }

      state('keyboardModifiers')[modifier] = true
    ...
    getActiveModifiers: () => {
      return _.reduce(state('keyboardModifiers'), (memo, isActivated, modifier) => {
        if (isActivated) {
          memo.push(modifier)
        }
    ...
    resetModifiers (doc) {
      ...
      for (let modifier in state('keyboardModifiers')) {
        const isActivated = state('keyboardModifiers')[modifier]

        state('keyboardModifiers')[modifier] = false
        if (isActivated) {
...
module.exports = { create, mixinModifiers }
